...
const OpeningPage = () => {
  ...
  <div className="flex-1 flex flex-col justify-center items-center px-2 sm:px-4 md:px-10 lg:px-36 py-4 md:py-8">
    <div className="w-full max-w-7xl shadow-2xl p-2 sm:p-4 md:p-10 lg:p-14 flex flex-col md:flex-row items-center relative" style={{backdropFilter:'blur(8px)'}}>
      {/* Left: Text */}
  ...
  <p className="text-sm sm:text-base md:text-lg text-[#b6a6e6] max-w-lg">
        ...
        <Link to='/ai'>
  <button style={{padding:"12px 40px"}} className="mt-4 px-6 sm:px-10 py-2 sm:py-3 border-2 border-white text-white rounded-full text-base sm:text-lg font-semibold hover:bg-white hover:text-[#2a0a2a] transition-all shadow-md">Get Started</button>
      </Link>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative mt-8 md:mt-0 w-full">
        <img src="/images/robot.png" alt="Robot" className="w-[220px] sm:w-[300px] md:w-[420px] lg:w-[500px] object-contain drop-shadow-2xl" style={{filter:'brightness(1.2)'}} />
  <div className="absolute top-4 right-0 md:static md:mt-8 flex flex-col items-start md:items-end w-full md:w-auto z-20">
          ...
          <button className="mt-2 sm:mt-4 flex items-center gap-1 sm:gap-2 text-[#e0d6f7] hover:text-white text-sm sm:text-base">
            <span className="border border-[#e0d6f7] rounded-full p-1 sm:p-2 flex items-center justify-center">
              <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polygon points="10,8 16,12 10,16" fill="#e0d6f7"/></svg>
            </span>
