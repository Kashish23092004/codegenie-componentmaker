...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
  const res = await fetch('https://codegenie-componentmaker-backend.onrender.com/signup', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#1a1440] via-[#0e0a1a] to-[#2a0a2a] px-2 sm:px-4">
      <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 md:p-10 rounded-2xl shadow-2xl flex flex-col gap-5 w-full max-w-md border border-gray-200">
        ...
        <button type="submit" className="mt-2 w-full py-3 rounded-lg text-lg font-semibold bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-md hover:from-purple-600 hover:to-indigo-700 transition-all">Sign Up</button>
